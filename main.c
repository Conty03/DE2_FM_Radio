 ...
 */ 
 ...
 uint8_t Debounce(uint8_t pressedButton) {
  ...
  if ((pressedButton == 1 && oldDebounce !=1) || debounceTimer == 1) {
    ...
    if (debounceTimer == 8) { // condition if the button is not pressed properly
    ...
    if (debounceTimer == 1) { // condition if the button is not released properly
 ...
 {    
   ...
   if ((newD & (1 << PD2)) == 0 &&
     ...
     if (nonDebouncedPD3Pressed != 1) {    // prevent conflict when both buttons are pressed
