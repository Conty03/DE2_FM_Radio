...
{	
  ...
  while (1) 
  {
    ...
    /*
    sprintf(actFreqConst, "%.1f", actFreq);
    u8g2_DrawStr(&u8g2, 10, 40, actFreqConst);
    u8g2_SendBuffer(&u8g2);
    */
  }
...
/*
uint8_t Debouncer(uint8_t button) {
...
ISR(TIMER1_OVF_vect)
{
  ...
  if (buttonPressedLong == 0 && buttonPressedLong2 == 0) {    // button pressed shortly - no frequency change yet
    ...
    if (initTime > 5) {   // after every 7th overflow (approx. 200 ms)
      slowFreq = 1;
      initTime = 0;

      if (fastTime > 3) {   // after maxInitTime * maxFastTime overflows of timer1
